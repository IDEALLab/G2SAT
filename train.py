...
def train(args, loader_train, loader_test, model, optimizer, writer_train, writer_test, device, save_dir='model/'):
    ...
    for epoch in range(args.epoch_num):
        ...
        for data in loader_train:
            ...
            label_positive = torch.ones(
                [
                    data.node_index_positive.shape[1],
                ], dtype=pred.dtype)
            label_negative = torch.zeros(
                [
                    data.node_index_negative.shape[1],
                ], dtype=pred.dtype)
...
def test(args, generator_list, model, repeat=0, outdir='graphs/'):
    ...
    for i, generator in enumerate(generator_list[:20]):  
        ...
        while True:
            ...
            # print('out:', out.shape, out)  # shape = (45, 32)
            # print('generator.data.node_index:', generator.data.node_index.shape,
            #       generator.data.node_index)  # shape = (2, x). E.g. (2, 5)
            # TODO: what is the original G2SAT behavior here? Why does LENS data lead to a (2, 0) gene.data.node_index?
            ...
            # print(nodes_first.shape, nodes_second.shape)  # E.g. (5, 32)
            pred = torch.sum(nodes_first * nodes_second, dim=-1)
            # print('pred', pred)
            ...
            # print('number of nodes:', generator.graph.number_of_nodes())
    ...
    print('Num {}, Node {} {} {}, Edge {} {} {}'.format(
        len(graphs), min(node_nums), max(node_nums),
