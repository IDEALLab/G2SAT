...
def load_graphs_lcg_LENS(data_dir, stats_dir):
    # load stats
    with open(stats_dir + 'circuit_stats.csv') as csvfile:
        data = csv.reader(csvfile, delimiter=',')
        stats = []
        for stat in data:
            stats.append(stat)
    ...
    for filename in os.listdir(data_dir):
        ...
        found = False
        for stat in stats:
            # print(filename, stat[0])
            if filename == stat[0]:
                found = True
                # print('file {} found in circuit_stats.csv'.format(filename))
                n = graph.number_of_nodes()
                n_elements = int(stat[1])
                n_ports = int(stat[2])
                if graph.number_of_nodes() != n_elements + n_ports:
                    ...
                    nodes_par2 = list(range(n_elements, n_elements + n_ports))
                    nodes_par1s.append(nodes_par1)
                    nodes_par2s.append(nodes_par2)
                    graphs.append(graph)
                break
        if not found:
            print('File {} not found in circuit_stats.csv!'.format(filename))
            ...
            while True:
                ...
                if np.random.rand() < self.yield_prob:
                    ...
                    if self.speedup:
                        ...
                        nodes_sub1 = set(
                            dict(nx.single_source_shortest_path_length(graph, node, cutoff=self.hop)).keys())
                        nodes_sub2 = set(
                            dict(nx.single_source_shortest_path_length(graph, node_new, cutoff=self.hop)).keys())
                        nodes_sub3 = set(
                            dict(nx.single_source_shortest_path_length(graph, node_sample, cutoff=self.hop)).keys())
                        ...
                        node_index_negative = torch.from_numpy(np.array([[mapping[node]],
                                                                         [mapping[node_sample]]])).long()
                    else:
                        ...
                        if self.simple_sample:
                            ...
                            node_index_negative = torch.from_numpy(np.array([[node_neg_pair[0]],
                                                                             [node_neg_pair[1]]])).long()
                        else:
